...
const registro_compra_cliente = async function(req, res){
    if(req.user){
        ...
        detalles.forEach(async element => {
            ...
            var element_producto = await Producto.findById({_id:element.producto});
            var new_stock =element_producto.stock - element.cantidad;

            await Producto.findByIdAndUpdate({_id: element.producto},{stock: new_stock});
            //Limpiar carrito
            await Carrito.remove({cliente:data.cliente});
            // await Carrito.findByIdAndDelete({cliente:data.cliente});
        });
